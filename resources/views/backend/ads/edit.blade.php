    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="mb-8 border-b border-slate-100 pb-4">
            ...
            <div class="flex items-center gap-2.5">
                ...
                <h2 class="text-2xl font-extrabold text-slate-800 tracking-tight">แก้ไขข้อมูลป้ายประชาสัมพันธ์</h2>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div class="lg:col-span-8">
                <form action="{{ route('backend.ads.update', $ad->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 md:p-8 rounded-2xl border border-slate-100 shadow-[0_8px_30px_rgba(0,0,0,0.02)] space-y-5">
                    @csrf @method('PUT')
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="title" id="in-title" value="{{ $ad->title }}" required class="w-full px-4 py-3 bg-slate-50/50 border border-slate-100 rounded-xl text-xs font-medium outline-none focus:bg-white focus:border-[#5EBEE6] focus:ring-4 focus:ring-[#5EBEE6]/10 transition-all text-slate-700">
                    </div>

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            <label class="block text-xs font-bold text-slate-500 pl-0.5">ลิงก์ปลายทางเมื่อกดป้าย URL</label>
                            <input type="url" name="link_url" id="in-url" value="{{ $ad->link_url }}" class="w-full px-4 py-3 bg-slate-50/50 border border-slate-100 rounded-xl text-xs font-medium outline-none focus:bg-white focus:border-[#5EBEE6] focus:ring-4 focus:ring-[#5EBEE6]/10 transition-all text-slate-700">
                        </div>
                        <div class="space-y-1.5">
                            ...
                            <div class="relative">
                                <select name="status" required class="appearance-none w-full px-4 py-3 bg-slate-50/50 border border-slate-100 rounded-xl text-xs font-bold text-slate-600 outline-none focus:bg-white focus:border-[#5EBEE6] focus:ring-4 focus:ring-[#5EBEE6]/10 cursor-pointer transition-all">
                                    <option value="active" {{ $ad->status == 'active' ? 'selected' : '' }}>เปิดการแสดงผลสาธารณะ (Active)</option>
                                    <option value="inactive" {{ $ad->status == 'inactive' ? 'selected' : '' }}>ซ่อนการใช้งานชั่วคราว (Inactive)</option>
                                </select>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <textarea name="description" id="in-desc" rows="3" class="w-full px-4 py-3 bg-slate-50/50 border border-slate-100 rounded-xl text-xs font-medium outline-none focus:bg-white focus:border-[#5EBEE6] focus:ring-4 focus:ring-[#5EBEE6]/10 min-h-[90px] resize-none transition-all text-slate-700">{{ $ad->description }}</textarea>
                    </div>

                    <div class="space-y-1.5">
                        <label class="block text-xs font-bold text-slate-500 pl-0.5">เปลี่ยนรูปภาพแบนเนอร์ (ปล่อยว่างไว้ถ้าใช้รูปเดิม)</label>
                        <input type="file" name="image" id="in-img" accept="image/*" class="w-full px-4 py-2.5 bg-white border border-slate-100 rounded-xl text-xs text-slate-400 font-medium file:mr-4 file:py-1.5 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-blue-50 file:text-[#5EBEE6] hover:file:bg-blue-100/50 cursor-pointer transition-all shadow-sm">
                    </div>
                    ...
                    <div class="pt-5 border-t border-slate-50 flex gap-3">
                        <button type="submit" class="flex-1 bg-slate-900 hover:bg-slate-800 text-white py-3 rounded-xl text-xs font-bold shadow-md transition-all active:scale-95">
                            <i class="fa-regular fa-floppy-disk mr-1"></i> อัปเดตข้อมูล
                        </button>
                ...
                <div class="bg-white rounded-2xl border border-slate-100 p-4 shadow-lg space-y-3">
                    <div class="aspect-[16/9] bg-slate-50 border border-slate-100 rounded-xl overflow-hidden relative">
                        ...
                        <div id="pv-no-img" class="w-full h-full flex flex-col items-center justify-center text-slate-300 {{ $firstImg ? 'hidden' : '' }}">
                            ...
                            <span class="text-[10px] font-bold text-slate-400">ไม่มีรูปภาพ</span>
                        </div>
                    </div>
                    <div class="space-y-2 pt-1">
                        <h3 id="pv-title" class="text-sm font-bold text-slate-800 line-clamp-1 leading-snug">{{ $ad->title }}</h3>
                        <p id="pv-desc" class="text-[11px] text-slate-400 line-clamp-2 font-medium leading-relaxed">{{ $ad->description }}</p>
                    </div>
                </div>
            </div>
        </div>
