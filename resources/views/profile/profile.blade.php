...
<div class="font-mitr text-slate-700 max-w-7xl mx-auto">
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-4 space-y-4">
            <div class="bg-white rounded-xl border border-gray-100 p-6 flex flex-col items-center shadow-sm">
                <div class="relative mb-4">
                    <div class="w-32 h-32 rounded-full border-2 border-[#5EBEE6]/20 overflow-hidden shadow-sm">
                        {{-- ดึงรูปโปรไฟล์จริง หรือใช้ UI Avatars ถ้าไม่มี --}}
                        <img src="{{ Auth::user()->profile_photo_url ?? 'https://ui-avatars.com/api/?name='.urlencode(Auth::user()->name).'&background=EBF4FF&color=5EBEE6' }}" 
                ...
                <button type="button" class="w-full py-1.5 border border-gray-200 rounded-full text-xs font-medium text-gray-500 hover:bg-gray-50 transition-colors mb-6">
                    เปลี่ยนรูปโปรไฟล์
                </button>
                ...
                <div class="w-full space-y-1">
                    ...
                    <div class="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden">
                        <div class="w-[85%] h-full bg-[#5EBEE6] rounded-full shadow-[0_0_8px_rgba(94,190,230,0.4)]"></div>
                    </div>
                </div>
            </div>
        ...
        <div class="lg:col-span-8">
            ...
            <form action="{{ route('profile.update') }}" method="POST" class="bg-white rounded-xl border border-gray-100 p-8 shadow-sm h-full">
                @csrf
                @method('PUT') {{-- ใช้ PUT สำหรับการอัปเดตข้อมูล --}}
                
                <div class="flex justify-between items-center mb-8">
                    <h2 class="text-lg text-slate-700">ข้อมูลส่วนตัว</h2>
                    <button type="submit" class="px-5 py-1.5 bg-[#5EBEE6] text-white rounded-lg text-xs hover:bg-[#4fb1d8] transition-all">
                        บันทึกข้อมูล
                    </button>
                </div>
                ...
                <div class="grid grid-cols-12 gap-x-6 gap-y-5">
                    ...
                    <div class="col-span-12 md:col-span-4 space-y-5">
                        <div class="space-y-1.5">
                            ...
                            <select name="prefix" class="w-full bg-white border border-gray-200 text-xs rounded-lg px-3 py-2 outline-none focus:border-[#5EBEE6] transition-all">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <select name="grade_level" class="w-full bg-white border border-gray-200 text-xs rounded-lg px-3 py-2 outline-none focus:border-[#5EBEE6] transition-all">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <select name="favorite_subject" class="w-full bg-white border border-gray-200 text-xs rounded-lg px-3 py-2 outline-none focus:border-[#5EBEE6] transition-all">
                    ...
                    <div class="col-span-12 md:col-span-8">
                        <div class="grid grid-cols-2 gap-4">
                            <div class="space-y-1.5">
                                ...
                                <input type="text" name="first_name" value="{{ old('first_name', Auth::user()->first_name) }}" placeholder="ชื่อจริง"
                            ...
                            <div class="space-y-1.5">
                                ...
                                <input type="text" name="last_name" value="{{ old('last_name', Auth::user()->last_name) }}" placeholder="นามสกุล"
                                    class="w-full border border-gray-100 text-xs rounded-lg px-3 py-2 outline-none focus:border-[#5EBEE6]">
                            </div>
                            
                            <div class="space-y-1.5">
                                <label class="text-[11px] text-gray-400">ชื่อเล่น</label>
                                <input type="text" name="nickname" value="{{ old('nickname', Auth::user()->nickname) }}" placeholder="ชื่อเล่น"
                                    class="w-full border border-gray-100 text-xs rounded-lg px-3 py-2 outline-none focus:border-[#5EBEE6]">
                            </div>
                            <div class="space-y-1.5">
                                <label class="text-[11px] text-gray-400">รหัสประจำตัว</label>
                                <input type="text" name="student_id" value="{{ old('student_id', Auth::user()->student_id) }}" placeholder="รหัสประจำตัว"
                                    class="w-full border border-gray-100 text-xs rounded-lg px-3 py-2 outline-none focus:border-[#5EBEE6]">
                            </div>
                            ...
                            <div class="col-span-2 space-y-1.5">
                                ...
                                <div class="text-right text-[9px] text-gray-300">{{ strlen(Auth::user()->bio) }} / 200</div>
                            </div>

                            <div class="space-y-1.5">
                                <label class="text-[11px] text-gray-400">โรงเรียน</label>
                                <select name="school_name" class="w-full border border-gray-100 text-xs rounded-lg px-3 py-2 outline-none focus:border-[#5EBEE6]">
                            ...
                            <div class="space-y-1.5">
                                ...
                                <select name="dream_university" class="w-full border border-gray-100 text-xs rounded-lg px-3 py-2 outline-none focus:border-[#5EBEE6]">
